...
namespace AC
{
	...
	public class MainCamera : MonoBehaviour
	{
		...
		private Vector2 startSensorSize;
		private Vector2 startLensShift;
		#endif
		...
		public void _LateUpdate ()
		{
			...
			if (attachedCamera && (!(attachedCamera is GameCamera25D)))
			{
				if (!isSmoothChanging)
				{
					...
					#if ALLOW_PHYSICAL_CAMERA
					if (attachedCamera._camera.usePhysicalProperties)
					{
						_camera.sensorSize = attachedCamera._camera.sensorSize;
						_camera.lensShift = attachedCamera._camera.lensShift;
					}
					#endif
				}
				else
		...
		private void UpdateCameraTransition ()
		{
			...
			if (retainPreviousSpeed && previousAttachedCamera != null)
			{
				...
				#if ALLOW_PHYSICAL_CAMERA
				startSensorSize = previousAttachedCamera._camera.sensorSize;
				startLensShift = previousAttachedCamera._camera.lensShift;
				#endif
			}
			...
			if (attachedCamera._camera.usePhysicalProperties)
			{
				_camera.sensorSize = Vector2.Lerp (startSensorSize, attachedCamera._camera.sensorSize, timeValue);
				_camera.lensShift = Vector2.Lerp (startLensShift, attachedCamera._camera.lensShift, timeValue);
			}
			#endif
		}
		...
		public void SnapToAttached ()
		{
			if (attachedCamera && attachedCamera._camera)
			{
				...
				#if ALLOW_PHYSICAL_CAMERA
				_camera.usePhysicalProperties = attachedCamera._camera.usePhysicalProperties;
				if (attachedCamera._camera.usePhysicalProperties)
				{
					_camera.sensorSize = attachedCamera._camera.sensorSize;
					_camera.lensShift = attachedCamera._camera.lensShift;
		...
		private void SmoothChange (float _changeTime, MoveMethod method, AnimationCurve _timeCurve = null)
		{
			...
			#if ALLOW_PHYSICAL_CAMERA
			startSensorSize = _camera.sensorSize;
			startLensShift = _camera.lensShift;
			#endif
