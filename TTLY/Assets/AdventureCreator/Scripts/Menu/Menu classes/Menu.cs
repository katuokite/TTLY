...
namespace AC
{
	...
	public class Menu : ScriptableObject
	{
		...
		public void ShowGUI ()
		{
			...
			else if (appearType == AppearType.WhenSpeechPlays)
			{
				...
				if (speechMenuType == SpeechMenuType.SpecificCharactersOnly)
				{
					...
					EditorGUILayout.LabelField ("Character(s) to limit to:", GUILayout.MaxWidth (148f));
				...
				{
					EditorGUILayout.BeginHorizontal ();
					EditorGUILayout.LabelField ("Character(s) to exclude:", GUILayout.MaxWidth (148f));
					limitToCharacters = CustomGUILayout.TextArea (limitToCharacters, GUILayout.MaxWidth (1000f), apiPrefix + ".limitToCharacters");
					EditorGUILayout.EndHorizontal ();
					EditorGUILayout.HelpBox ("Multiple character names should be separated by a colon ';'", MessageType.Info);
				}
		...
		{
			if (appearType == AppearType.Manual || appearType == AppearType.OnInputKey || appearType == AC.AppearType.OnInteraction || appearType == AppearType.OnContainer || appearType == AppearType.MouseOver || appearType == AppearType.OnViewDocument)
			{
				return true;
			}
			return false;
