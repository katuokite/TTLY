		...
		private void ShowActiveInputsGUI ()
		{
			...
			if (selectedActiveInput != null && settingsManager.activeInputs.Contains (selectedActiveInput))
			{
				...
				if (showSelectedActiveInput)
				{
					...
					selectedActiveInput.label = EditorGUILayout.TextField ("Label:", selectedActiveInput.label);
					selectedActiveInput.inputName = EditorGUILayout.TextField ("Input button:", selectedActiveInput.inputName);
					selectedActiveInput.inputType = (SimulateInputType) EditorGUILayout.EnumPopup ("Input type:", selectedActiveInput.inputType);
					if (selectedActiveInput.inputType == SimulateInputType.Axis)
					{
						selectedActiveInput.axisThreshold = EditorGUILayout.Slider ("Axis threshold:", selectedActiveInput.axisThreshold, -1f, 1f);
					}
					selectedActiveInput.enabledOnStart = EditorGUILayout.Toggle ("Enabled by default?", selectedActiveInput.enabledOnStart);
					selectedActiveInput.gameState = (GameState) EditorGUILayout.EnumPopup ("Available when game is:", selectedActiveInput.gameState);
					selectedActiveInput.actionListAsset = ActionListAssetMenu.AssetGUI ("ActionList when triggered:", selectedActiveInput.actionListAsset, "", defaultName);
				}
				EditorGUILayout.EndVertical ();
			}
		}
