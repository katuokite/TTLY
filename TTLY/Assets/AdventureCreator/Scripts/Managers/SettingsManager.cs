	...
	public class SettingsManager : ScriptableObject
	{
		...
		private void ShowSaveGameSettings ()
		{
			...
			if (showSave)
			{
				...
				saveFileName = CustomGUILayout.DelayedTextField ("Save filename:", saveFileName, "AC.KickStarter.settingsManager.saveFileName");
				if (saveFileName != "")
				{
		...
		private void ShowInterfaceSettings ()
		{
			...
			if (showInterface)
			{
				...
				if (CanUseCursor ())
				{
					if (interactionMethod == AC_InteractionMethod.ChooseHotspotThenInteraction)
					{
						...
						if (selectInteractions != SelectInteractions.CyclingCursorAndClickingHotspot)
						{
							...
							{
								autoHideInteractionIcons = CustomGUILayout.ToggleLeft ("Auto-hide Interaction icons based on Hotspot / item?", autoHideInteractionIcons, "AC.KickStarter.settingsManager.autoHideInteractionIcons");
							}
							else
							{
				...
				allowGameplayDuringConversations = CustomGUILayout.ToggleLeft ("Allow regular gameplay during Conversations?", allowGameplayDuringConversations, "AC.KickStarter.settingsManager.allowGameplayDuringConversations");
		...
		private void ShowMovementSettings ()
		{
			...
			if (showMovement)
			{
				if (movementMethod == MovementMethod.FirstPerson)
				{
					...
					if (inputMethod != InputMethod.TouchScreen)
					{
						dragWalkThreshold = CustomGUILayout.FloatField ("Max free-aim speed:", dragWalkThreshold, "AC.KickStarter.settingsManager.dragWalkThreshold");
					}
				}
